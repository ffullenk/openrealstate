...
class viewPdfComponent extends CWidget {
	...
	public function getFile($id) {
		$filePdf = $this->pdfCachePath.'/'.$this->filePrefix . $id . '-'. Yii::app()->language. '.pdf';
	...
	public function generateFile($id, $fromAdmin = false) {		
		...
		if (!$fromAdmin) {
			if (!$apartment->active || !$apartment->owner_active) {
...
//		}
		...
		$file_name = $this->pdfCachePath.'/'.$this->filePrefix . $id . '-'. Yii::app()->language. '.pdf';

...
class Menu extends ParentModel{
			...
			foreach($menuItems as $item){
				...
				if($item['type'] == self::LINK_DROPDOWN){
						...
						if($tmpItem['subitems'] == $item['id']){
							if($tmpItem['type'] == self::LINK_DROPDOWN_MANUAL){
								...
								if (param('useBootstrap'))
									$subitem['linkOptions'] = array('class'=>'admMenuSeondLevel');
					...
					if($subitems){
						...
						if (param('useBootstrap'))
							$menu = CMap::mergeArray ($menu, $subitems);
	...
	public static function getWidgetOptions(){
		$arrWidgets =  array(
			'' => tc('No'),
			'news' => tc('News'),
			'apartments' => tc('List ads'),
			'viewallonmap' => tc('Search for ads on the map'),
			'contactform' => tc('The form of the "Contact Us"'),
			'randomapartments' => tc('List ads (random)'),
		);

...
class SiteController extends Controller {
	...
	/*public function actionTest(){
		...
		$sql = 'SELECT id, message FROM {{translate_message}} WHERE status=0 OR status=1';
		$result = Yii::app()->db->createCommand($sql)->queryAll();

		foreach($result as $item){
			$res = system('find ./ -name \'*.php\' -exec grep -ch "\''.preg_quote($item['message']).'\'" {} \; | awk \'{sum += $0} END {print sum}\'');
			if($res != 0){
				$sql = 'UPDATE {{translate_message}} SET status=4 WHERE id="'.$item['id'].'"';
				$result = Yii::app()->db->createCommand($sql)->execute();
			} else {
				$sql = 'UPDATE {{translate_message}} SET status=3 WHERE id="'.$item['id'].'"';
				$result = Yii::app()->db->createCommand($sql)->execute();
			}
		}
	}*/
